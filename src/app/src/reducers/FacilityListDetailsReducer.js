...
const startConfirmOrRejectMatchOrRemoveItem = state =>
    update(state, {
...
const failConfirmOrRejectMatchOrRemoveItem = (state, payload) =>
    update(state, {
        confirmOrRejectMatchOrRemoveItem: {
            fetching: { $set: false },
            error: { $set: payload },
...
const toggleSelectedRowIndex = (state, payload) => {
    const newRowIndex =
        state.selectedFacilityListItemsRowIndex === payload
...
const completeConfirmOrRejectMatchOrRemoveItem = (state, payload) => {
    const indexForListItemToUpdate = state.items.data.findIndex(
        ({ id }) => id === payload.id,
    );
...
export default createReducer(
    {
        ...
        [failFetchFacilityListItems]: (state, payload) =>
            update(state, {
                items: {
                    $merge: {
                        data: null,
        ...
        [completeFetchFacilityListItems]: (state, payload) =>
            update(state, {
        ...
        [startAssembleAndDownloadFacilityListCSV]: state =>
            update(state, {
                downloadCSV: {
                    data: { $set: null },
                    fetching: { $set: true },
        ...
        [failAssembleAndDownloadFacilityListCSV]: (state, payload) =>
            update(state, {
                downloadCSV: {
                    fetching: { $set: false },
                    error: { $set: payload },
        ...
        [completeAssembleAndDownloadFacilityListCSV]: (state, payload) =>
            update(state, {
                downloadCSV: {
                    data: { $set: payload },
                    fetching: { $set: false },
