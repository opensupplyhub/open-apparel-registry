...
export default createReducer({
    [startFetchDashboardUpdateLocationContributors]: state => update(state, {
        contributors: {
            fetching: { $set: true },
            error: { $set: null },
        },
    }),
    [failFetchDashboardUpdateLocationContributors]: (state, payload) => update(state, {
        contributors: {
            fetching: { $set: false },
            error: { $set: payload },
        },
    }),
    [completeFetchDashboardUpdateLocationContributors]: (state, payload) => update(state, {
        contributors: {
            fetching: { $set: false },
            error: { $set: null },
            data: { $set: payload },
    ...
    [startFetchUpdateLocationFacility]: state => update(state, {
        fetching: { $set: true },
        error: { $set: initialState.error },
        data: { $set: initialState.data },
    }),
    [failFetchUpdateLocationFacility]: (state, error) => update(state, {
        fetching: { $set: initialState.fetching },
        error: { $set: error },
    }),
    [completeFetchUpdateLocationFacility]: (state, data) => update(state, {
        fetching: { $set: initialState.fetching },
        data: { $set: data },
    }),
    [clearUpdateLocationFacility]: state => update(state, {
        oarID: { $set: initialState.oarID },
        data: { $set: initialState.data },
        fetching: { $set: initialState.fetching },
        error: { $set: initialState.error },
    }),
    [updateUpdateLocationFacilityOARID]: (state, oarID) => update(state, {
        oarID: { $set: oarID },
        error: { $set: initialState.error },
    }),
    [updateUpdateLocationLat]: (state, lat) => update(state, {
        newLocation: {
            lat: { $set: lat },
        },
    }),
    [updateUpdateLocationLng]: (state, lng) => update(state, {
        newLocation: {
            lng: { $set: lng },
        },
    }),
    [updateUpdateLocationNotes]: (state, notes) => update(state, {
        notes: { $set: notes },
    }),
    [startUpdateFacilityLocation]: state => update(state, {
        update: {
            fetching: { $set: true },
            error: { $set: initialState.update.error },
        },
    }),
    [completeUpdateFacilityLocation]: (state, data) => update(state, {
        update: {
    ...
    [failUpdateFacilityLocation]: (state, error) => update(state, {
        update: {
            fetching: { $set: initialState.update.fetching },
            error: { $set: error },
        },
    }),
    [updateUpdateLocationContributor]: (state, contributor) => update(state, {
