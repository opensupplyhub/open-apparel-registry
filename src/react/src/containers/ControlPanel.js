...
class ControlPanel extends PureComponent {
  ...
  componentDidMount() {
    ...
    .then(data => {
      const sources = data.sources.filter(s => s.name).map(({ name, _id, uid, list, user_type }) => ({ name, _id, uid, list, user_type }))
    ...
    .then(response => response.json())
    .then(data => {
      const countryNames = data.countries.map(s => {
  ...
  sortedCountries = () => this.state.countries.sort((a, b) => {
    const aName = a.names ? a.names[0] : a.name
    const bName = b.names ? b.names[0] : b.name
    return aName.toLowerCase().localeCompare(bName.toLowerCase())
  }).map(c => (
    <MenuItem value={ c.code } key={ c.code }>
      <Checkbox color="primary" checked={ this.state.country.indexOf(c.code) > -1 } />
      <ListItemText primary={ c.names ? c.names[0] : c.name } />
    </MenuItem>
  ))

  checkboxSources = () => this.state.sources
  .sort((a, b) => a.name.toLowerCase().localeCompare(b.name.toLowerCase()))
  .map(c => (
    <MenuItem value={ c._id } key={ c._id }>
      <Checkbox color="primary" checked={ this.state.contributor.indexOf(c._id) > -1 } />
      <ListItemText primary={ c.name } className="notranslate" />
    </MenuItem>
  ))

  sourcesTypes = () => _.uniqBy(this.state.sources
  .filter(s => (s.user_type !== null && s.user_type !== undefined))
  .sort((a, b) => a.user_type.toLowerCase().localeCompare(b.user_type.toLowerCase())), 'user_type')
  .map(c => (
    <MenuItem value={ c.user_type } key={ c.user_type }>
      <Checkbox color="primary" checked={ this.state.contributorType.indexOf(c.user_type) > -1 } />
  ...
  // parameters
  handleSharedUrl = ({ name, contributor, country, factory }) => this.setState({
    name: name || '',
    contributor: (contributor && contributor.split(',')) ? contributor.split(',') : [],
    country: (country && country.split(',')) ? country.split(',') : [],
    tabValue: 1
  ...
  searchFactory = (e, specificFactory) => {
    ...
    let url = `${process.env.REACT_APP_API_URL}/searchFactoryNameCountry?name=${name}&country=${country}`
    ...
    .then(data => {
      if (!data || !Array.isArray(data) || !data.length || data.length <= 0) {
        this.setState({ factories: [], factoriesRes: this.createfactoriesRes([]), factoriesList: [], isSpinning: false })
        this.props.onUpdate([])
        return
      }
      ...
      let factories = !name ? data : data.filter(f => _.includes(f.name.toLowerCase(), name.toLowerCase()) || _.includes(f.otherNames, name))
      ...
      if (contributor && contributor.length > 0) {
        ...
        factories = factories.filter(f => {
          ...
          return _.difference(contributor, fSourceId).length === 0
        })
      }
      ...
      if (contributorType && contributorType.length > 0) {
        factories = factories.filter(f => {
          ...
          return _.difference(contributorType, fSourceTypes).length === 0
        })
      }
      ...
      this.setState({ factories, factoriesRes, factoriesList, isSpinning: false }, () => {
  ...
  createfactoriesRes = factories => {
    ...
    if (!this.state.name && !this.state.contributor.length && !this.state.country.length && !this.state.contributorType.length) {
      return (
        <div className="control-panel__group">
          <h1 className="control-panel__heading">SEARCH RESULTS:</h1>
          <p className="helper-text">Your search criteria was too broad. Results are limited to { factories.length } Facilities of { totalFactories } Total Facilities.</p>
        </div>
      )
    }
    ...
    return (
      <div className="control-panel__group">
        <h1 className="control-panel__heading">SEARCH RESULTS:</h1>
        <p className="helper-text">Found { factories.length } Facilities of { totalFactories } Total Facilities.</p>
      </div>
    )
  }
  ...
  createfactoriesList = factories => {
    ...
    if (process.env.REACT_APP_CLICKABLE_LIST) return factories.map((f, i) => <p className={ (i === factories.length - 1) ? 'notranslate link-underline cursor margin-bottom-64' : 'notranslate link-underline cursor' } role="presentation" onClick={ () => this.onSelectFactory(f.uniqueId) } key={ f.uniqueId }>{ f.name }</p>)
    return factories.map((f, i) => <p className={ (i === factories.length - 1) ? 'notranslate margin-bottom-64' : 'notranslate' } key={ f.uniqueId }>{ f.name }</p>)
  }
  ...
  factoriesCSV = () => parse(this.props.factories, { fields: ['nameId', 'longitude', 'latitude', 'name', 'address'] })
  ...
  render() {
    ...
    const selectFac = selectedFactory ? factories.find(f => f.uniqueId === selectedFactory) : null

    const { tabValue, factoriesRes, factoriesList, showFacroiesList } = this.state

    return (
      <Container>
        <ShowOnly if={ !!selectedFactory }>
          <FactoryInfo info={ selectFac } onClearSelection={ this.onChildClear } />
        </ShowOnly>
        <ShowOnly if={ !selectedFactory }>
          <div className="panel-header">
            <h3 className="panel-header__title">Open Apparel Registry</h3>
            <p className="panel-header__subheading">The open map of global apparel factories.</p>
          </div>
            ...
            <TabContainer>
              <div className="control-panel__content">
                <p className="control-panel__body">
                  The Open Apparel Registry (OAR) is a tool to identify every
                  apparel facility worldwide. It is an open sourced, global
                  database of apparel (garments, footwear and accessories)
                  facility names and locations. The tool  normalizes uploaded
                  data - specifically facility names and addresses - submitted
                  by contributors such as brands, industry associations and
                  non-profits and assigns each facility a unique OAR ID number.
                  Each contributor is listed next to every facility it has submitted.
                </p>
                <p className="control-panel__body">
                  To contribute to the database, users must create an account.
                  Anyone can sign up and contribute. Users interested in browsing
                  the OAR are able to access it freely without creating an account.
                </p>
                <a href="http://info.openapparel.org/about" target="_blank" rel="noopener noreferrer" className="link-underline">Learn More</a>
              </div>
            </TabContainer>
          </ShowOnly>
              ...
              <div className="control-panel__content">
                <form onSubmit={ this.searchFactory }>
                  ...
                  <FormControl style={{ width: '100%', marginBottom: '32px' }}>
                    ...
                    <Select
                      ...
                      input={ <Input name="contributor" id="contributor" className="notranslate" /> }
                  ...
                  <FormControl style={{ width: '100%', marginBottom: '32px' }}>
                    ...
                    <Select
                      ...
                      className="full-width margin-top-16 margin-bottom-16 notranslate"
                      input={ <Input name="contributorType" id="contributorType" className="notranslate" /> }
                  ...
                  <div className="form__action">
                    <a className="control-link" href="mailto:info@openapparel.org?subject=Reporting an issue">
                      Report an issue
                    </a>
                    <div className="offset offset-right">
                      ...
                      { this.state.isSpinning
                        ? <CircularProgress size={ 30 } className="margin-left-16" />
                        : (
                          <Button
                            variant="contained"
                            size="small"
                ...
                <ShowOnly if={ this.state.factories.length > 0 }>
                  <div className="control-panel__action control-panel__action--center">
                    <div className="offset offset-right">
                      <Button
                        ...
                        disableRipple
                      >
                        <ShowOnly if={ !showFacroiesList }>VIEW </ShowOnly>
                        <ShowOnly if={ showFacroiesList }>HIDE </ShowOnly>
                        FACILITY LIST
                      </Button>
                      <Button
                        ...
                        color="primary"
                        disableRipple
                        onClick={ () => DownloadCSV(this.factoriesCSV(), 'factories.csv') }
                      >
                        DOWNLOAD CSV
                      </Button>
                    </div>
                  ...
                  <ShowOnly if={ showFacroiesList }>
                    <div className="control-panel__scroll">{ factoriesList }</div>
                  </ShowOnly>
                </ShowOnly>
